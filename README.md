...
但这样的数据依然方向是有问题的，本类以此为基准，在构造函数、getChangePostionSlices和getMultiplePositionSlices等方法中，可以进行自动扶正（为position参数提供指定字符串，获取对应断面的情况），但如果是不同排列的数据则需要调整rot90和flip参数进行扶正。
